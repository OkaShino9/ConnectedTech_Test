...
def main():
    ...
    input_source = st.sidebar.radio("Image Source", ["Use Example Images", "Upload Your Own Image"])
    
    image = None
    img_path = None
    uploaded_file = None
    
    if input_source == "Use Example Images":
        ...
        st.sidebar.info("Upload your own image for detection.")
        ...
        with col2:
            ...
            if st.button("Detect and Count Bottles", type="primary"):
                with st.spinner('Detecting objects...'):
                    # Preprocess image
                    if uploaded_file is not None:
                        # Convert PIL Image to cv2 required format
